...
apastat <- function(test, roundN = 2, es = TRUE, ci = TRUE, var = NULL) {
  if (class(test) == "htest") {
    ...
    if (grepl("t-test", test$method)) {
      out <- paste0(
        "t(", format(round(test$parameter, roundN), roundN), ")=", format(round(test$statistic, roundN), roundN),
    ...
    if (grepl("Pearson", test$method)) {
      out <- paste0(
        ...
        ", t(", round(test$parameter, roundN), ")=", format(round(test$statistic, roundN), roundN),
        ifelse(ci == TRUE,
    ...
    if (grepl("Spearman", test$method)) {
      out <- paste0(
        "\u03C1", "=", sub(x = format(round(test$estimate, roundN), roundN), "0.", "."),
    ...
    if (grepl("Kendall", test$method)) {
      out <- paste0(
        "\u03C4", "=", sub(x = format(round(test$estimate, roundN), roundN), "0.", "."),
  ...
  # lm
  if (class(test) == "lm" | class(test) == "summary.lm") {
    if (class(test) == "lm") {
      tmp <- summary(test)
    } else {
      tmp <- test
    ...
    if (is.null(var)) {
      fp <- pf(tmp$fstatistic[1], tmp$fstatistic[2], tmp$fstatistic[3], lower.tail = F)
      out <- paste0(
        "N=", NROW(tmp$residuals),
        ", F(", tmp$fstatistic[2], ",", tmp$fstatistic[3], ")=", format(round(tmp$fstatistic[1], roundN), roundN),
      ...
      out <- paste0(
        ...
        ", t(", round(tmp$fstatistic[3], roundN), ")=", format(round(coef(tmp)[varx, "t value"], roundN), roundN),
